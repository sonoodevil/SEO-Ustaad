...
export const CurriculumView: React.FC<CurriculumViewProps> = ({
  ...
  const [showReward, setShowReward] = useState<{ score: number; title: string } | null>(null);
  ...
  const handleSubmitQuiz = () => {
    ...
    if (scorePercent >= 80) {
      triggerConfetti();
      setShowReward({ score: scorePercent, title: currentWeek.title.en });
    }
  };
        ...
        {showReward && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <motion.div
              ...
              className="bg-[#0F0F0F] border-2 border-blue-500/50 rounded-3xl p-8 max-w-sm w-full shadow-2xl shadow-blue-500/20 relative text-center overflow-hidden"
            >
              {/* Background Glow */}
              <div className="absolute top-0 left-1/2 -translate-x-1/2 w-40 h-40 bg-blue-600/30 blur-[60px] rounded-full -z-10" />
              
              <button
                onClick={() => setShowReward(null)}
                className="absolute top-4 right-4 p-1.5 rounded-full bg-white/5 hover:bg-white/10 text-white/40 hover:text-white transition"
              >
                <X className="w-4 h-4" />
              </button>
              ...
              <div className="flex justify-center mb-6">
                <div className="relative">
                  <motion.div
                    ...
                    className="w-20 h-20 bg-blue-600 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-500/40"
                  >
                  ...
                  >
                    <Star className="w-4 h-4 text-[#0F0F0F] fill-current" />
                  </motion.div>
                </div>
              </div>
              ...
              <p className="font-urdu-body text-blue-400 text-lg mb-4">
                شاندار کارکردگی! آپ نے ٹیسٹ پاس کر لیا
              </p>
              ...
              <p className="text-xs text-white/60 leading-relaxed mb-6">
                Congratulations! Your deep understanding of <span className="text-white font-bold">{showReward.title}</span> shows you're well on your way to becoming an SEO Ustaad.
              </p>

              <button
                onClick={() => setShowReward(null)}
                className="w-full py-3.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-sm font-bold shadow-lg shadow-blue-500/30 transition transform hover:scale-[1.02] active:scale-[0.98]"
              >
                Keep Learning (جاری رکھیں)
              </button>
            </motion.div>
          </div>
        )}
